...
public abstract class GSYVideoPlayer extends GSYBaseVideoPlayer implements View.OnClickListener, View.OnTouchListener, SeekBar.OnSeekBarChangeListener, TextureView.SurfaceTextureListener {
    ...
    //这是一个比较智障的做法哈
    private static boolean canSpeed = true;

    /**
     *
     * 设置是否可以快进
     *
     *  一个是进度条 一个是 onThouch事件
     *
     */
    public void setCanSpeed(boolean canSpeed){
        this.canSpeed = canSpeed;

        if(mProgressBar instanceof DontSeekProgressBar){
            DontSeekProgressBar progressBar = (DontSeekProgressBar) mProgressBar;
            progressBar.canDrage = canSpeed;
        }
        ...
        if (id == R.id.surface_container) {
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    Log.i("xxxxxxxx","canSpeed  "+canSpeed + "    "+mChangePosition);
                    if (mChangePosition && canSpeed) {
    ...
    public void onCompletion() {
        ...
        if(videoProgressChangeLisener != null){
            videoProgressChangeLisener.onComplition();
        }
    }
    ...
    protected void setProgressAndTime(int progress, int secProgress, int currentTime, int totalTime) {
        ...
        if(videoProgressChangeLisener != null){
            videoProgressChangeLisener.changeLisener(currentTime,totalTime);
        }
    ...
    /**
     *
     * 使用静态主要是因为屏幕切换之后对象是重新创建的对象
     *
     */
    private static VideoProgressChangeLisener videoProgressChangeLisener;

    public void setVideoProgressChangeLisener(VideoProgressChangeLisener videoProgressChangeLisener) {
        this.videoProgressChangeLisener = videoProgressChangeLisener;
    }

    public interface VideoProgressChangeLisener{
        void changeLisener(int current, int total);
        void onComplition();//播放完成回调
    }
